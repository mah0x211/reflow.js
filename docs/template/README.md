...
- **Scopes** resolve identifiers through three explicit symbols: `$` (globals), `@name` (a named `x-data`), and `.name` (the nearest lexical binding). See [Scope resolution](./scopes.md).
...
|---|---|---|
| Scope | [`x-data`](./directives/x-data.md) | Declare one or more named scopes reachable as `@name`. |
...
- **`x-data` may combine with any other directive.**
...
3. **Directive parse** — each `x-*` attribute is parsed into its own structure (`x-data` → JSON5 scopes, `x-for` → range, `x-each` → iteration binding, everything else → an expression AST).
...
5. **Validate** — orphan `x-elseif`/`x-else`/`x-case`/`x-nocase`, illegal directive combinations, unregistered helper references, and `x-break` outside a loop all raise `ReflowCompileError` here.
...
2. **Walk** — the IR is traversed depth-first. Each element pushes any `x-data` frame, then either iterates itself (`x-for` / `x-each`), branches (`x-if` chain, `x-match`), or emits directly.
...
5. **Include** — `x-include` renders the target template with the same globals but a fresh lexical scope, respecting the include-depth and cycle guards.
